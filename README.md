...
## 완료되면 Codex가 예약을 직접 삭제합니다

**사용자가 예약을 찾아 수동 삭제할 필요가 없습니다.** v0.4.0 앱 모드에서는 Codex가 완료 기준과 테스트를 확인하고, 결과를 저장한 다음 자신에게 연결된 예약을 삭제합니다. 이미 요청한 자동 완료 정리에 대해 별도 확인을 반복해서 요구하지 않습니다.

`개발 → 테스트·완료 기준 확인 → 완료 기록 저장 → Codex가 연결된 예약 삭제 → 삭제 성공 확인·기록`

- 삭제 대상은 이 프로젝트에 연결된 예약 하나입니다. 소스·작업 대화·체크포인트·로그·완료 근거는 보존합니다.
- 실제 삭제는 Codex 앱의 예약 관리 도구가 수행하고, Python 도우미는 확인된 결과를 기록합니다. 이 두 단계 모두 Codex가 처리하므로 사용자가 도우미 명령을 실행할 필요는 없습니다.
...
**실제 삭제 검증 완료 (2026-09-18):** 현재 Codex 앱에서 실행되지 않는 PAUSED 테스트 예약을 생성하고, 같은 작업에서 삭제를 호출해 `deleteStatus: deleted` 응답을 확인했습니다. 이는 현재 호스트의 직접 삭제 기능 검증이며, 모든 호스트 버전이나 실제 프로젝트의 완료 판정까지 보장하는 것은 아닙니다. 자세한 내용은 [검증 기록](docs/VERIFICATION.md)을 참고하세요.
